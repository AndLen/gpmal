    ...
    public static void initialise(List<Instance> instances, boolean doNNs) {
        ...
        switch (dissimMapToUse) {
            ...
            case "fractional0.1":
                ...
                break;
            case "none":
                DEFAULT_MAP = new DissimilarityMap() {
                    @Override
...
//                }
    ...
    public static void writeToFile(double[][] outputs, String fileName) {
        List<String> lines = stream(outputs).map(DatasetUtils::arrayToString).collect(toList());
        try {
    ...
    private static String arrayToString(double[] output) {
        return stream(output).mapToObj(String::valueOf).collect(joining(","));
    }
